...
THICKNESS = 0.05  # controls how thick the frustum's 'bars' are
...
class CameraTrajectoryBuffer:
    ...
    def __init__(self,
        ...
        # add current estimated pose to the trajectory
        self.trajectory_est = []
        self.trajectory_est_previous = None

    def grow_camera_path(self, new_camera, camera_est):
        """
        ...
        if self.trajectory_previous is not None:
            ...
            if 0.0001 < current_dist < line_skip:
                ...
                # add line for the estimated trajectory
                est_current_pos = camera_est[:3, 3]
                line_cuboid_est = cuboid_from_line(line_start=self.trajectory_est_previous,
                                                     line_end=est_current_pos,
                                                     color=(255, 0, 0))
                
                self.trajectory_est.append(line_cuboid_est)
            else:
        ...
        self.trajectory_est_previous = camera_est[:3, 3]
    ...
    def get_mesh(self):
        ...
        trajectory_est_mesh = self.trajectory_est + self.frustums
        trajectory_est_mesh = trimesh.util.concatenate(trajectory_est_mesh)
        trajectory_est_mesh = pyrender.Mesh.from_trimesh(trajectory_est_mesh)

        return trajectory_mesh, self.frustum_images, trajectory_est_mesh